...
download:{[startdate;enddate;currencypairs]
	.lg.o[`download;" " sv ("Running download function for";
		$[1=count currencypairs;string currencypairs;", " sv string currencypairs];"between";string startdate;"and";string enddate)];
  ...
  // Download any available files not already downloaded in the date range      
	{if[not x in filetab[`names];
                .lg.o[`download;"Downloading ",1_string x];
			.[{x 1:.Q.hg y};(x;y);.lg.e[`download;"Remote service unavailable"]];
			$[2000<hcount x;[`filetab upsert (x;y;hcount x;.z.p);.lg.o[`download;1_(string x)," downloaded successfully"]];
				[hdel x;.lg.o[`download;1_(string x)," is empty, file deleted"]]]]}'[names;hsym urls];
